...
def main():
    ...
    for a3m_path in tqdm(a3m_files):
        if output_dir:
            # Keep the relative path structure and convert to lowercase
            rel_path = a3m_path.relative_to(input_dir)
            # Convert the filename part to lowercase while preserving the directory structure
            rel_path_parts = list(rel_path.parts)
            rel_path_parts[-1] = rel_path_parts[-1].lower()
            out_path = output_dir.joinpath(*rel_path_parts)
