...
class Boltz1(LightningModule):
    def __init__(  # noqa: PLR0915, C901, PLR0912
        ...
        # Use RNA-specific input embedder for RNA features
        self.input_embedder = RNAInputEmbedder(**full_embedder_args)
        ...
        # Use RNA-specific distogram module
        self.distogram_module = RNADistogramModule(token_z, num_bins)
    ...
    def forward(
        ...
        with torch.set_grad_enabled(
            ...
            # Get input embeddings and potential RNA base-pairing information
            embedder_output = self.input_embedder(feats)
            if isinstance(embedder_output, tuple) and len(embedder_output) == 2:
                # RNAInputEmbedder returns (s_inputs, base_pairing)
                s_inputs, base_pairing = embedder_output
                print(f"✓ Using RNA base-pairing constraints from RNAInputEmbedder")
            else:
                # Standard embedder just returns s_inputs
                s_inputs = embedder_output
                base_pairing = None
            ...
            for i in range(recycling_steps + 1):
                ...
                with torch.set_grad_enabled(self.training and (i == recycling_steps)):
                    ...
                    if not self.no_msa:
                        ...
                        try:
                            # Pass the base pairing information to the RNAMSAModule if available
                            if base_pairing is not None:
                                print("Using RNA base-pairing constraints in MSA module")
                                msa_output = self.msa_module(z, s_inputs, feats, base_pairing=base_pairing)
                            else:
            ...
            # Pass RNA structure information to the distogram module if available
            rna_structure_info = {}
            if base_pairing is not None:
                rna_structure_info["base_pairing"] = base_pairing
                pdistogram = self.distogram_module(z, rna_structure_info)
            else:
